...
from domain.order.controller.rest.v1.docs.get_overdue_orders import GET_OVERDUE_ORDERS_RESPONSES
...
@router.get(
    '/overdue/',
    responses=GET_OVERDUE_ORDERS_RESPONSES,
)
async def get_overdue_orders(
    registry: Annotated[Container, Depends(get_registry)],
    response: Response,
) -> list[OverdueOrderOutput] | None:
    """
    Returns all overdue orders.
    """
    if not (orders := await registry.resolve(IOrderRepository).get_overdue_orders()):
        response.status_code = status.HTTP_204_NO_CONTENT
    return orders
